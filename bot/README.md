...
- Listener lưu metadata **ảnh/video** theo user, thời điểm, loại và số lượng; không lưu URL/file media.
- Có thể sao chép message Zalo live sang một **Telegram forum topic hoặc channel cụ thể**; đích forward tách khỏi chat duyệt cleanup.
- Theo dõi thành viên join/leave; chủ động sync snapshot member định kỳ để sửa lệch khi admin xoá tay hoặc WebSocket rớt event.
- Ghi lịch sử `member_sync_runs` và `member_events` để dashboard hiển thị lần sync gần nhất và audit join/leave/remove.
- Ghi `bot_health` và hỗ trợ `check-permissions` không phá huỷ để kiểm role/quyền bot.
...
| `npm run dev` | Chạy listener keep-alive từ TypeScript khi phát triển local. |
| `npm run build && npm start` | Build rồi chạy listener từ `dist/`. PM2 production dùng luồng này. |
...
| `npm run sync-members` | Đồng bộ danh sách member hiện tại từ Zalo về DB. Dùng khi cần sửa lệch ngay sau khi xoá tay. |
| `npm run check-permissions` | Kiểm tra role/quyền bot trong group, không kick/xoá thật. |
| `npm run health-check` | Cron kiểm heartbeat + WebSocket bot và gửi Telegram khi lỗi/hồi phục. |
...
npm run dev
# Mở web /login để tạo và quét QR.

npm run list-groups
# Copy GROUP_ID nhóm cần quản lý vào .env rồi restart bot.
```
...
### Forward Zalo sang một Telegram topic/channel

Telegram gọi các “channel con” trong forum group là **topic**. Bot sao chép nội
dung từ Zalo (không phải native forward vì message gốc không nằm trên Telegram),
và dùng `message_thread_id` để chọn đúng topic.

1. Thêm bot Telegram vào supergroup/channel và cấp quyền gửi message/media.
2. Với forum topic, gửi một message mới vào topic; với channel, đăng một post mới.
   Sau đó chạy ngay:

```bash
npm run telegram-find-topic
```

Nếu cron `telegram-poll` lấy update trước, tạm dừng cron/PM2 cron, gửi lại một
message mới và chạy lại lệnh. Copy các ID được in ra vào `.env`:

```dotenv
TELEGRAM_FORWARD_ENABLED=1
TELEGRAM_FORWARD_BOT_TOKEN=token-của-bot-forward-riêng
TELEGRAM_FORWARD_CHAT_ID=-1001234567890
TELEGRAM_FORWARD_TOPIC_ID=456
```

Nếu đích là channel/chat thường, dùng ID dạng `-100...` hoặc `@channel_username`
và để trống `TELEGRAM_FORWARD_TOPIC_ID`. Sau đó kiểm tra và restart listener:

```bash
npm run telegram-forward-test
pm2 restart zalo-bot --update-env   # production
```
...
2. Cron `health-check` kiểm heartbeat mỗi 5 phút và báo Telegram nếu bot stale.
3. Cron `sync-votes` đồng bộ voter trong poll mỗi 6 giờ (backup cho sync trong listener).
4. Ngày 25, cron chạy `cleanup-warn` và gửi cảnh báo vào group.
5. Ngày 3, cron chạy `monthly-cleanup` và gửi danh sách qua Telegram.
...
- Listener không gửi gì khi forward đang tắt và không tự kick; kick chỉ xảy ra ở `monthly-cleanup` khi `DRY_RUN=0`.
- Khi bật `TELEGRAM_FORWARD_ENABLED=1`, listener gửi bản sao message sang Telegram; cân nhắc quyền riêng tư của thành viên trước khi bật.
