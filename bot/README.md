...
- `telegram-poll`: xử lý nút Duyệt/Huỷ, timeout tự động sau 48h, và `/retry` nếu kick lỗi giữa chừng.
- Nếu chưa cấu hình Telegram mà `DRY_RUN=0`, `monthly-cleanup` chạy xoá thật ngay theo CLI.
...
# Chạy thật không Telegram: kick ngay theo danh sách
DRY_RUN=0 npm run monthly-cleanup
```

Nếu dùng Telegram approval, điền `TELEGRAM_BOT_TOKEN` và `TELEGRAM_CHAT_ID`, rồi chạy:

```bash
DRY_RUN=0 npm run monthly-cleanup   # gửi danh sách + nút Duyệt/Huỷ
npm run telegram-poll               # nên chạy bằng cron mỗi phút
```
